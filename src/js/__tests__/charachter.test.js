...
test('getSpecials', () => {
    const newCharacter = new Character ("Лучник", "Bowman");
        newCharacter.health = 50;
        newCharacter.level = 3;
        newCharacter.attack = 40;
        newCharacter.defence = 10;
        newCharacter.setSpecial([
            {
                id: 8,
                name: 'Двойной выстрел',
    ...
    expect(newCharacter.getFields({ special: newCharacter.specials })).toEqual(correct);

})

test('specialsEmpty', () => {
    const character = new Character("Лучник", "Bowman");
    expect(() => character.getFields()).toThrow("У персонажа нет атак!");
})
